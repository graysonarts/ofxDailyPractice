...
void JzAzBzColor::from(ofxXmlSettings& settings, const string& name) {
    value[0] = settings.getValue(name + ":Jz", 0.);
    value[1] = settings.getValue(name + ":Az", 0.);
    value[2] = settings.getValue(name + ":Bz", 0.);
}